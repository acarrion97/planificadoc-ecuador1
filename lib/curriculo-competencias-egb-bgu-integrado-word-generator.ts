...
function nombreMateria(ambitos: AmbitoDesarrollo[]): string {
  for (const a of ambitos) {
    const ce = buscarCompetenciaEspecificaEGBBGU(a.competenciaCodigo || "");
    if (ce) {
      const materia = MATERIAS_EGB_BGU.find((m) => m.competencias.some((c) => c.codigo === ce.codigo));
...
export async function generarCurriculoCompetenciasWordEGBBGUIntegrado(
  ...
  const ambitosConLabels = ambitos.map((a) => {
  ...
  const COL_SEM = Math.floor(TW * 0.1);
  const COL_DUA = Math.floor(TW * 0.5);
  const COL_REC = Math.floor(TW * 0.2);
  const COL_TECH = TW - COL_SEM - COL_DUA - COL_REC;
    ...
    makeTable(
      [
        new TableRow({
          tableHeader: true,
          children: [
            tc([p("Semana", { bold: true, size: 8, color: WHITE })], COL_SEM, { bg: COLOR_PRIMARY }),
      ...
      TW,
      [COL_SEM, COL_DUA, COL_REC, COL_TECH]
    )
  );
  ...
  for (let semana = 1; semana <= numSemanasCalc; semana++) {
    for (const ambito of ambitos) {
      ...
      if (clase && (clase.inicio?.length || clase.desarrollo?.length || clase.cierre?.length)) {
        ...
        const descCorta = ce!.descripcion.length > 100 ? ce!.descripcion.substring(0, 100) : ce!.descripcion;
        ...
        duaContent.push(p(`Sugerencias para el inicio:`, { bold: true, size: 7 }));
        duaContent.push(p(`• Presentar la actividad mediante una dinámica relacionada con: ${indSample.substring(0, 80)}`, { size: 7 }));
        ...
        duaContent.push(p(`• Aplicar una actividad práctica orientada a: ${indSample.substring(0, 80)}`, { size: 7 }));
        duaContent.push(p(`Sugerencias para el cierre:`, { bold: true, size: 7 }));
