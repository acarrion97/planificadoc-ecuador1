...
/**
 * Genera filas para la sección de Adaptaciones Curriculares en el PCA Trimestral.
...
function crearAdaptacionesPcaTrimestral(adaptaciones: AdaptacionCurricular[]): TableRow[] {
  ...
  for (const adap of activas) {
    ...
    filas.push(new TableRow({
      children: [
        makeCell({
          paragraphs: [textPara(`${codigoLabel} — ${tipoNombre} — ${gradoInfo.nombre}`, true, SZ7, AlignmentType.LEFT)],
          span: 7,
          width: COL_TOTAL,
          bg: BG_ADAPT_STUDENT,
          vAlign: VerticalAlign.CENTER,
    ...
    if (adap.descripcionNecesidad) {
      infoLeft.push(new Paragraph({
        ...
        children: [run(adap.descripcionNecesidad, false, SZ6)],
      }));
    }
    ...
    if (adap.indicadoresAdaptados?.length) {
      infoRight.push(new Paragraph({
        ...
        children: [run("Indicadores adaptados:", true, SZ7)],
      }));
          ...
          makeCell({
            paragraphs: [
              ...
              ...adap.adaptacionesAcceso.map(acc =>
                new Paragraph({
                  spacing: { after: 6 },
                  children: [run(`• ${acc.descripcion}`, false, SZ6)],
                })
              ),
            ],
            span: 7,
      ...
      adap.adaptacionesPorDia.forEach((dp, idx) => {
        ...
        if (dp.adaptacionERCA) {
          ...
          for (const key of faseKeys) {
            const cfg = ERCA_COLORS.find(c => c.key === key);
            if (cfg) {
        ...
        children: [
          makeCell({
            paragraphs: [
              ...
              new Paragraph({
                spacing: { after: 6 },
                children: [run(adap.seguimiento, false, SZ6)],
              }),
            ],
            span: 7,
            width: COL_TOTAL,
        ...
        children: [
          makeCell({
            paragraphs: [
              ...
              new Paragraph({
                spacing: { after: 6 },
                children: [run(adap.observaciones, false, SZ6)],
              }),
            ],
            span: 7,
            width: COL_TOTAL,
        ...
        children: [
          makeCell({
            paragraphs: [
              ...
              new Paragraph({
                spacing: { after: 6 },
                children: [run((adap as any).notaDIAC, false, SZ6)],
              }),
            ],
            span: 7,
            width: COL_TOTAL,
...
export async function generarWordPcaTrimestral(formData: any, aiResult: any): Promise<Blob> {
  ...
  // ── Adaptaciones curriculares ──
  const adaptacionesRows = crearAdaptacionesPcaTrimestral(formData.adaptacionesCurriculares || []);
