...
Responde ÚNICAMENTE con JSON: { "parejasSugeridas": [ { "destrezaFocoCodigo": "string", "destrezaFocoDescripcion": "string", "sugerenciaEnfoque": "string (1 oración concreta de cómo el estudiante más consolidado puede apoyar al compañero en esta destreza)" } ] }`;
  ...
  sugerirProyecto: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const prompt = `Eres un experto en el programa "Conecta, Nivela y Crea" del MinEduc Ecuador (Semanas 4-5 — CREA). El proyecto interdisciplinario se aplica al finalizar el trimestre y corresponde a la evaluación sumativa; debe derivarse coherentemente del diagnóstico de Semana 1 y reforzar las destrezas de nivelación.
