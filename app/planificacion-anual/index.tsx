...
export default function PlanificacionAnualScreen() {
  ...
  const [semanasTotal, setSemanasTotal] = useState("40");
  const [semanasEval, setSemanasEval] = useState("8");
  ...
  const [unidades, setUnidades] = useState<Unidad[]>([
    { id: generateUnitId(), numero: 1, dcdsSeleccionadas: [], duracionSemanas: 6 },
  ]);
  ...
  const [bibliografiaDocente, setBibliografiaDocente] = useState("");
  ...
  // ── DCD disponibles según selección ──
  ...
  // ── Grados disponibles según subnivel ──
  ...
  const addUnidad = useCallback(() => {
    setUnidades(prev => [
      ...prev,
      { id: generateUnitId(), numero: prev.length + 1, dcdsSeleccionadas: [], duracionSemanas: 4 },
    ]);
  }, []);
  ...
  // ── Cambiar área con advertencia si hay DCD seleccionadas ──
  const handleAreaChange = useCallback((newArea: string) => {
    const anySelected = unidades.some(u => u.dcdsSeleccionadas.length > 0);
    if (anySelected) {
      const msg = "Cambiar el área borrará las destrezas seleccionadas en todas las unidades. ¿Deseas continuar?";
      if (Platform.OS === "web") {
  ...
  const handleSubnivelChange = useCallback((v: string) => {
    ...
    if (anySelected) {
      const msg = "Cambiar el subnivel borrará las destrezas seleccionadas. ¿Continuar?";
      if (Platform.OS === "web") {
        if (!confirm(msg)) return;
      } else {
  ...
  const handleGenerar = useCallback(async () => {
    ...
    try {
      ...
      const formData = {
        ...
        semanasTrabajoTotal: parseInt(semanasTotal) || 40,
        semanasEvaluacion: parseInt(semanasEval) || 8,
        ...
        bibliografiaDocente: bibliografiaDocente.trim(),
      ...
      if (result.success && result.pcaId) {
        ...
        Alert.alert("Error", result.error || "No se pudo generar la PCA. Intenta de nuevo.");
      }
    } catch (err: any) {
      Alert.alert("Error", err.message || "Error de conexión. Verifica tu internet.");
    }
  }, [
    institucion, docente, area, subnivel, grado, anioLectivo, paralelo,
    cargaHoraria, semanasTotal, semanasEval, usaEjes, ejesSeleccionados,
    unidades, metodologias, tecnicas, bibliografiaDocente,
    firmaElab, firmaElabFecha, firmaRev, firmaRevFecha, firmaApro, firmaAproFecha,
  ]);

  const isGenerating = generatePca.isPending;
  ...
  return (
    <ScreenContainer>
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
        ...
        {/* DUA informativo */}
        <View style={[styles.duaBar, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text style={[styles.duaTitle, { color: colors.foreground }]}>Principios DUA integrados</Text>
          <View style={styles.duaRow}>
            <View style={[styles.duaDot, { backgroundColor: "#EC4899" }]} />
            <Text style={[styles.duaLabel, { color: colors.muted }]}>Representación ¿qué?</Text>
          </View>
          <View style={styles.duaRow}>
            <View style={[styles.duaDot, { backgroundColor: "#1E3A5F" }]} />
            <Text style={[styles.duaLabel, { color: colors.muted }]}>Acción y Expresión ¿cómo?</Text>
          </View>
          <View style={styles.duaRow}>
            <View style={[styles.duaDot, { backgroundColor: "#22C55E" }]} />
            <Text style={[styles.duaLabel, { color: colors.muted }]}>Implicación ¿por qué?</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          <FieldLabel label="Objetivos del área" colors={colors} tipo="B" />
          ...
          <View style={{ height: 8 }} />
          <FieldLabel label="Objetivos del grado / curso" colors={colors} tipo="B" />
          <View style={[styles.iaField, { borderColor: "#97C459" }]}>
            <Text style={[styles.iaPlaceholder, { color: "#5A8A1F" }]}>
              Se generarán al presionar "Generar vista previa"
            </Text>
        ...
        <View style={styles.section}>
          <View style={{ flexDirection: "row", gap: 10 }}>
            ...
            <View style={{ flex: 1 }}>
              <FieldLabel label="Semanas de trabajo" colors={colors} />
              <TextInput
                style={[styles.input, { color: colors.foreground, borderColor: colors.border }]}
                value={semanasTotal} onChangeText={setSemanasTotal}
                keyboardType="number-pad" placeholder="40" placeholderTextColor={colors.muted}
              />
            </View>
          </View>
          ...
          <TextInput
            ...
            keyboardType="number-pad" placeholder="8" placeholderTextColor={colors.muted}
          />
        ...
        {/* ── SECCIÓN 5: Unidades ── */}
        <SectionTitle numero="5" titulo="Unidades de planificación" colors={colors} />
        <View style={styles.section}>
        ...
        {/* ── SECCIÓN 6: Metodologías y evaluación ── */}
        <SectionTitle numero="6" titulo="Metodologías y evaluación" colors={colors} />
        <View style={styles.section}>
        ...
        {/* ── SECCIÓN 7: Bibliografía ── */}
        <SectionTitle numero="7" titulo="Bibliografía" colors={colors} />
        <View style={styles.section}>
          <FieldLabel label="Bibliografía del docente (normas APA 7.ª ed.)" colors={colors} />
          <TextInput
            style={[styles.textarea, { color: colors.foreground, borderColor: colors.border }]}
            value={bibliografiaDocente}
            onChangeText={setBibliografiaDocente}
            multiline numberOfLines={4}
            placeholder="Ministerio de Educación. (2023). Currículo Nacional Priorizado. Quito: MinEduc."
            placeholderTextColor={colors.muted}
            textAlignVertical="top"
          />
          <View style={{ height: 8 }} />
          <FieldLabel label="Bibliografía sugerida" colors={colors} tipo="B" />
          <View style={[styles.iaField, { borderColor: "#97C459", minHeight: 48 }]}>
            <Text style={[styles.iaPlaceholder, { color: "#5A8A1F" }]}>La IA añadirá referencias adicionales</Text>
          </View>
        </View>

        {/* ── SECCIÓN 8: Firmas ── */}
        <SectionTitle numero="8" titulo="Firmas de aprobación" colors={colors} />
        <View style={styles.section}>
          <View style={{ flexDirection: "row", gap: 10 }}>
            {[
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 8 }}>
          ...
          >
            {isGenerating ? (
              <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                ...
                <Text style={styles.btnGenerarText}>Generando tu PCA...</Text>
              </View>
            ) : (
              <Text style={styles.btnGenerarText}>🚀 Generar vista previa de mi PCA</Text>
            )}
          </Pressable>
          {isGenerating && (
          ...
          <Text style={[styles.freeHint, { color: colors.muted }]}>
            ✨ La generación es gratuita. Solo pagas si decides descargar el documento completo.
          </Text>
        </View>
      </ScrollView>
    </ScreenContainer>
...
const styles = StyleSheet.create({
  ...
  duaTitle: { fontSize: 12, fontWeight: "700", marginBottom: 6 },
  duaRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 3 },
  duaDot: { width: 12, height: 12, borderRadius: 2 },
  duaLabel: { fontSize: 11 },
  sectionHeader: {
    flexDirection: "row",
    alignItems: "center",
    paddingHorizontal: 16,
