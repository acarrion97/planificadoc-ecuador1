...
| `cnc-word-generator.ts` | Plan "Conecta, Nivela y Crea" (arranque del año escolar, 5 semanas) | **Normativa distinta** — no es el instructivo de PCA: [Lineamientos Pedagógicos Costa-Galápagos 2026-2027](https://educacion.gob.ec), sección 2.1 "Conecta, nivela y crea". Las columnas de las tablas semanales sí replican las del Anexo 1 | ✅ Verificado 2026-09-22 tras alinear la evaluación diagnóstica de la Semana 1 con "Herramientas sugeridas para la evaluación diagnóstica" (MinEduc 2026). Layout correcto (2 págs.), sin el bug de `columnWidths`. La técnica/instrumento de diagnóstico va en la columna ACTIVIDADES EVALUATIVAS, no en una fila propia (evita duplicarla). |
| `evaluacion-word-generator.ts` | Reporte de Evaluación Diagnóstica (preguntas con opciones visuales + clave, resultados, brechas, recomendaciones) | Sin anexo oficial: instrumento propio del módulo. En Node las opciones SVG (IA) salen como "[imagen]" porque no hay canvas; en el navegador se rasterizan a PNG | ✅ Layout verificado 2026-09-24 (1 pág. con 3 preguntas, imágenes JPEG/PNG en línea con la letra) |
